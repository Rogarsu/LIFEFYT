...
import { getActiveRoutine }             from '../../lib/database'
...
import type { WorkoutExercise, WorkoutDay, ExerciseSlot, GeneratedRoutine } from '../../types'
...
// ─── RoutinePage ──────────────────────────────────────────────────────────────
export function RoutinePage({ onStartSession, refreshKey: _refreshKey }: RoutinePageProps) {
  ...
  // Build exercise map at RoutinePage level for week view usage
  const exerciseMap = useMemo(
    () => new Map(EXERCISES.map(e => [e.id, e])),
    [],
  )
  ...
  // ── Program-based week view ────────────────────────────────────────────────
  if (program) {
    const daysPerWeek      = program.daysPerWeek
    const sessionsPerBlock = daysPerWeek * 4
    const currentBlock     = program.blocks.find(b => b.blockNumber === program.currentBlock) ?? program.blocks[0]
    const params           = currentBlock ? METHOD_PARAMS[currentBlock.method] : null

    // Current week number within the block (1-indexed)
    const currentWeekNum = Math.ceil(program.currentSession / daysPerWeek)

    // Session numbers that belong to this week
    const weekStart = (currentWeekNum - 1) * daysPerWeek + 1
    const thisWeekSessionNumbers = Array.from({ length: daysPerWeek }, (_, i) => weekStart + i)

    // Build WorkoutDay objects for each session this week
    const weekDays: WorkoutDay[] = thisWeekSessionNumbers.map(sessionNum => {
      const dayIndex = sessionToDayIndex(sessionNum, daysPerWeek)
      const day      = currentBlock?.days[dayIndex] ?? currentBlock?.days[0]

      const exercises: WorkoutExercise[] = (day?.slots ?? []).flatMap(slot => {
        const exId = resolveExerciseId(slot.exercisePool, program.currentBlock, sessionNum, sessionsPerBlock)
        const ex   = exId ? exerciseMap.get(exId) : undefined
        if (!ex) return []
        const sets = currentBlock?.setsOverride ?? ex.sets[program.experience]
        const reps = currentBlock?.repsOverride ?? ex.reps[program.experience]
        const rest = currentBlock?.restOverride ?? ex.rest[program.experience]
        return [{
          exercise: ex,
          sets,
          reps,
      ...
      const totalTime = Math.round(
        exercises.reduce((acc, e) => acc + (parseInt(e.sets) || 3) * (45 + e.rest) / 60, 0),
      )

      return {
        dayNumber: sessionNum,
        name:      day?.name  ?? `Sesión ${sessionNum}`,
        focus:     day?.focus ?? '',
        exercises,
        totalTime,
      }
    ...
    const currentSessionDay = weekDays[activeDay] ?? weekDays[0]

    const isSessionCompleted = (sessionNum: number) =>
      program.completedSessions.some(cs => cs.b === program.currentBlock && cs.s === sessionNum)

    const headerLabel = params
      ? `Semana ${currentWeekNum} · Bloque ${program.currentBlock} — ${params.label}`
      : `Semana ${currentWeekNum}`

    return (
      <div className="flex flex-col min-h-screen bg-dark-900 safe-top">
        {/* Header */}
        <div className="relative overflow-hidden bg-gradient-to-br from-brand-500/20 to-dark-900 pt-6 pb-5 px-5">
          <div className="relative">
            <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-1">Esta semana</p>
            <h1 className="text-xl font-black text-white leading-tight">{headerLabel}</h1>
          </div>
        </div>
        ...
        {view === 'week' && (
          <div className="px-5 pt-4 pb-2">
            <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${daysPerWeek}, minmax(0, 1fr))` }}>
              {weekDays.map((day, i) => (
                <DayCard
                  key={day.dayNumber}
                  day={day}
                  isActive={activeDay === i}
                  isCompleted={isSessionCompleted(day.dayNumber)}
        ...
        {view === 'week' && (
          <div className="flex-1 overflow-y-auto no-scrollbar px-5 pb-36">
            <AnimatePresence mode="wait">
              {currentSessionDay && (
                <motion.div
                  key={`${activeDay}-${currentSessionDay.dayNumber}`}
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }}
                  transition={{ duration: 0.22 }}
                >
                  {isSessionCompleted(currentSessionDay.dayNumber) ? (
                    /* ── Completed session view ── */
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                      <div className="py-4 flex items-center gap-2 mb-2">
                        <div className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center flex-shrink-0">
                          <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                          </svg>
                        </div>
                        <span className="text-green-400 text-xs font-black uppercase tracking-wider">Sesión completada</span>
                      </div>
                      <div className="flex flex-col gap-3 opacity-50">
                        {currentSessionDay.exercises.map((we, i) => (
                          <div
                            key={we.exercise.id}
                            className="flex items-center gap-4 glass rounded-2xl p-3"
                          >
                            <div className="relative w-16 h-16 rounded-xl overflow-hidden flex-shrink-0">
                              <img src={we.exercise.image} alt={we.exercise.nameEs} className="w-full h-full object-cover grayscale" />
                              <div className="absolute inset-0 bg-gradient-to-t from-dark-900/60 to-transparent" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-white/60 font-bold text-sm leading-tight truncate">{we.exercise.nameEs}</p>
                              <p className="text-white/30 text-xs mt-0.5 truncate">{we.exercise.primaryMuscles.join(', ')}</p>
                            </div>
                            <div className="text-right flex-shrink-0">
                              <p className="text-white/40 font-black text-sm tabular-nums">{we.sets}×{we.reps}</p>
                              <p className="text-white/20 text-xs">{we.rest}s</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <>
                      ...
                      <div className="py-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <h2 className="text-white font-black text-xl">{currentSessionDay.name}</h2>
                            <div className="flex items-center gap-3 mt-1">
                              <span className="glass px-2.5 py-1 rounded-full text-xs font-semibold text-white/60">
                                {currentSessionDay.focus}
                              </span>
                              <span className="text-white/30 text-xs">
                                {currentSessionDay.exercises.length} ej · ~{currentSessionDay.totalTime} min
                              </span>
                            </div>
                          </div>
                          <motion.button
                            whileTap={{ scale: 0.95 }}
                            onClick={() => handleStartWorkout(currentSessionDay)}
                      ...
                      <div className="glass rounded-2xl p-4 mb-4">
                        ...
                        <div className="space-y-2">
                          {Object.entries(
                            currentSessionDay.exercises.reduce((acc, we) => {
                            ...
                            .slice(0, 5)
                            .map(([muscle, sets]) => {
                              const maxSets = currentSessionDay.exercises.reduce((a, we) => Math.max(a, parseInt(we.sets.split('-')[0])), 0)
          ...
          <AnimatePresence>
            {selectedEx && (
              <ExerciseDetailModal
                ...
                total={currentSessionDay?.exercises.length ?? 0}
                onClose={() => setSelectedEx(null)}
                onPrev={selectedEx.idx > 0 ? () => setSelectedEx({ ex: currentSessionDay!.exercises[selectedEx.idx - 1], idx: selectedEx.idx - 1 }) : undefined}
                onNext={selectedEx.idx < (currentSessionDay?.exercises.length ?? 0) - 1 ? () => setSelectedEx({ ex: currentSessionDay!.exercises[selectedEx.idx + 1], idx: selectedEx.idx + 1 }) : undefined}
              />
            )}
          </AnimatePresence>
        )}
  ...
  // ── Fallback: old routine-based view (no program) ─────────────────────────
  if (!routine) {
    return (
